...
\titleformat{\chapter}[hang]
  ...
  {\prechaptername\thechapter\postchaptername}{1em}{}
...
近年，ChatGPT や Claude に代表される生成型人工知能は，自然言語処理（Natural Language Processing, NLP）の発展を大きく牽引している．これらのシステムは Transformer に基づく大規模言語モデル（Large Language Models, LLM）を中核とし，大規模コーパスに対する自己教師あり学習を通じて，多様なタスクにまたがる汎用的な言語能力を獲得している．

しかし，LLM はあくまで学習時点までのテキスト分布に依存した静的なパラメトリックモデルであり，以下のような限界を有する．第一に，学習後に新たに生じた事象や更新された知識を自動的に反映することができず，急速に変化するニュース，技術動向，法制度などに対して陳腐化した情報を出力しうる．第二に，内部に埋め込まれた知識は分散表現として暗黙的に符号化されているため，回答の根拠となる具体的な文書や出典を明示することが困難である．第三に，入力が曖昧な場合や学習分布から外れた問いに対して，もっともらしく見えるが事実に基づかない出力（ハルシネーション）が生じるという信頼性の問題がある．

この問題に対処する枠組みとして，外部知識を検索してから生成を行う検索拡張生成（Retrieval-Augmented Generation, RAG）が提案されている．RAG は，百科事典，ニュース記事，技術文書，企業内ナレッジベースなどの非パラメトリックな知識源から関連文書を検索し，その内容に基づいて LLM に回答を生成させることで，(i) 知識の更新容易性，(ii) 出典に基づく説明可能性，(iii) ハルシネーション抑制，を同時に狙う手法である．

しかし，既存の RAG 研究の多くは英語単言語環境を前提として設計・評価されている．現実世界の情報エコシステムは多言語的であり，質問文の言語と有力な証拠文書の言語が一致しない状況，すなわちクロスリンガル質問応答（cross-lingual QA）が一般的に発生する．このとき，以下の課題が顕在化する．

\begin{enumerate}
  \item \textbf{検索段階の言語バイアス}：多言語ベクトル検索モデルであっても，高資源言語（特に英語）やクエリ言語と同一の言語を選好する傾向があり，他言語に存在する重要情報が十分に想起されない場合がある．
  \item \textbf{翻訳に伴う情報損失}：質問言語と異なる証拠文書を一括してピボット言語（多くは英語）へ翻訳する手法は，言語統一による扱いやすさと引き換えに，翻訳誤りやニュアンス喪失，トークン長増大によるコスト増加を招く．特に低資源言語ペアにおいて訳質のばらつきが大きい．
  \item \textbf{出力言語制御の困難さ}：クロスリンガル環境では，モデルが英語や複数言語を混在させた回答を行う（code-switching）問題が頻発し，ユーザが期待する言語で一貫した回答を得られないことが報告されている．
  \item \textbf{証拠帰属の不透明さ}：多言語証拠と翻訳結果が混在する中で，どの主張がどの原文に基づくのかが不明瞭になり，検証可能性や監査性が損なわれる．
\end{enumerate}

これらの問題は，単に精度の低下にとどまらず，多言語話者間の情報アクセス格差を拡大しうるというフェアネス上の懸念とも直結する．特に，低資源言語話者が最新かつ信頼できる情報にアクセスする際，高資源言語への過度な依存や誤訳に起因する不利益を被る可能性がある．

先行研究は，多言語 RAG の課題に対して，(i) 多言語ベンチマークの整備，(ii) 英語への統一翻訳による単一言語化，(iii) 内部知識と外部証拠の統合，(iv) 弁証的推論に基づく多段階生成，など様々な方向からアプローチを試みている．しかし，「高カバレッジな多言語検索」，「部分的かつ信頼性を担保した翻訳」，「ユーザ言語での出力一貫性」，「主張と原文証拠の可逆的対応」という要件を，計算コストを抑えつつ同時に満たす統合的フレームワークは十分に確立されていない．

本研究は，このギャップに着目し，多言語環境において実運用可能な RAG システム LBRAG（Language-Bridged Retrieval-Augmented Generation）を提案する．LBRAG は，言語間の「橋渡し」として翻訳を利用しつつも，全文一括翻訳に依存せず，重要な証拠に限定した選択的翻訳と文単位アライメントにより，正確性・言語一貫性・コスト効率・追跡可能性のバランスを図ることを目指す．
...
\begin{enumerate}
  \item \textbf{正確性と高カバレッジの両立}：ユーザ質問の言語 $L_q$ と英語を含む多言語インデックスに対して並列検索を行い，多言語ベクトル検索とクロスリンガル再ランキングを組み合わせることで，言語バイアスを抑えつつ高カバレッジな候補証拠を取得する．
  \item \textbf{選択的翻訳によるコスト効率化}：全文書の統一翻訳ではなく，関連度・翻訳信頼度・トークン長に基づき重要段落のみを翻訳対象として選択する「選択的言語ブリッジ」を導入し，翻訳コストと情報損失を抑制しながら，多言語証拠を扱いやすい形に統合する．
  \item \textbf{追跡可能で言語一貫な回答生成}：翻訳後の文単位アライメントおよび数値・日付・固有名詞などのスロット整合性検証により，主張と原文証拠の対応関係を保持したまま，ユーザ言語 $L_q$ で一貫した回答を生成できるプロンプト設計と評価指標を整備する．
\end{enumerate}

これにより，LBRAG は多言語 RAG における「部分翻訳」「可逆的対応」「軽量かつ制御可能な生成」を統合的に実現し，既存手法の課題を補完することを目指す．
...
\begin{enumerate}
  \item \textbf{多言語混合検索と言語間リランキングの統合}：ユーザ言語インデックスと英語インデックスに対する並列検索，多言語ベクトル検索モデルおよびクロスリンガルクロスエンコーダを組み合わせた二段階スコアリングにより，高資源言語への過度な偏りを抑えつつ，多言語にまたがる高カバレッジな候補段落集合を構築する設計を示す．
  \item \textbf{選択的言語ブリッジの提案}：翻訳予算の制約下で，関連度・翻訳品質・トークンコストに基づく効率比により翻訳対象段落を貪欲選択する枠組みを導入する．さらに，翻訳後に原文との文単位アライメントと重要スロット整合性チェックを行うことで，翻訳誤差の影響を抑えつつ原文との可逆的対応と証拠帰属可能性を確保する．
...
これらの要素を組み合わせることで，本研究は，多言語 RAG における検索戦略・翻訳戦略・生成制御・評価指標を一体として設計した LBRAG フレームワークを提示し，クロスリンガル QA の実運用に向けた一つの具体的指針を与える．
...
第3章では，提案手法 LBRAG の詳細な構成を述べる．多言語混合検索，選択的言語ブリッジ，文単位アライメントおよびスロット整合性検証，生成プロンプト設計について定義する．
...
\begin{enumerate}
  \item \textbf{多言語混合検索＋クロスリンガル再ランキング}：ユーザ言語インデックスと英語インデックスに対して並列検索を行い，多言語ベクトル検索とクロスリンガルクロスエンコーダによる再ランキングを統合することで，高資源言語偏重を抑えつつ多言語証拠のカバレッジを確保する設計を採用する．
...
\begin{enumerate}
  \item 多言語混合検索（ユーザ言語および英語インデックスに対する並列検索とクロスリンガル再ランキング）
...
\begin{figure}[!htbp]
  ...
  \begin{tikzpicture}[
    ...
    % 多言語混合検索 内部
    \node[block, below=11mm of input] (dualret) {並列検索（$L_q$/英語）};
    \node[block, below=6mm of dualret] (rerank) {言語間リランキング};
...
まず，ユーザ質問 $q$（言語 $L_q$）に対して，多言語混合検索モジュールがユーザ言語インデックスと英語インデックスに対する並列検索と再ランキングを行い，上位 $K$ 件の候補段落集合 $D$ を構築する．
...
ユーザ質問を $q$，その言語を $L_q$ とする．多言語埋め込みモデル
（例：bge-m3，e5-multilingual，mContriever 等）をエンコーダ $E(\cdot)$ とし，
クエリおよび段落を単位ベクトルへ写像する：
...
ユーザ言語インデックス（$L_q$）および英語インデックスの双方から候補段落 $d_i$ を取得し，
それぞれについてコサイン類似度に基づく密検索スコア
\begin{equation}
  s_{\mathrm{dense}}(q,d_i) = \mathbf{v}_q^\top \mathbf{v}_{d_i}
\end{equation}
を計算する．この段階では，高リコールを重視し，多様な言語の候補を広く収集する．

\subsection{言語間クロスエンコーダによる再ランキング}

密検索スコアのみでは否定表現や細粒度の意味差異に敏感でないため，
クロスリンガルなクロスエンコーダにより，質問 $q$ と段落 $d_i$ の関連度
$s_{\mathrm{rerank}}(q,d_i)$ を再評価する．
...
さらに，検索結果の類似度が一様に低い場合や，
取得された段落の大部分が望ましい言語分布から著しく逸脱している場合には，
BM25 によるスパース検索やクエリ拡張を用いるフォールバック戦略を適用し，
多言語環境におけるカバレッジの安定性を高める．
...
その上で，文埋め込み類似度などを用いて，
原文文 $s$ と訳文文 $\tilde{s}$ の対応関係の集合
$\mathcal{A}_i = \{(s,\tilde{s})\}$ を構築する．
...
翻訳を行わなかった段落（$i \notin \Omega$）については，
原文のまま提示し，必要に応じてピボット言語側から「訳注」的に説明を補うことで，
過度な翻訳に起因する情報歪曲を回避する．
...
\begin{itemize}
  ...
  \item 「根拠となる文・段落を明示すること」
  \item 「矛盾する証拠が存在する場合は両者を提示した上で判断すること」
\end{itemize}
といった制約文を組み込むことで，
出力言語の一貫性と証拠帰属の明確化を図る．
...
\begin{enumerate}
  \item ユーザ質問 $q$（言語 $L_q$）を多言語エンコーダでベクトル化し，
        $L_q$ インデックスおよび英語インデックス上で並列検索を行う．
  \item 密検索およびクロスリンガルクロスエンコーダにより
        スコア $S_{\mathrm{retr}}$ を計算し，上位 $K$ 件の候補段落集合 $D$ を得る．
...
\begin{itemize}
  ...
  \item 検索：ベクトル内積（正規化）により Top-$K$ 候補を取得する．
  \item 再ランキング：LLM による listwise rerank を用いる（候補に対し 0--1 の関連度スコアを推定）．
...
\begin{itemize}
  \item 翻訳：LLM を用いた翻訳（数値・日付・固有名詞を保持する指示）を行う．
  \item pivot 言語：Full-Translate は常に pivot=en，LBRAG は候補分布に応じて pivot を選択し得る（実装既定）．
  \item 翻訳コスト：トークン数の近似として文字数から簡易推定を用いる（実装上の近似）．
  \item アライメント：文分割後，greedy に 1 対 1 対応を作成し，被覆率とスロット一致率を算出する．
\end{itemize}

\subsection{生成モデルとプロンプト}

\begin{itemize}
  \item 生成：多言語対応 LLM を用い，全手法で共通とする．
...
\[
\mathrm{CNBE}
= \frac{\mathrm{F1}_{\mathrm{system}} - \mathrm{F1}_{\mathrm{direct}}}{\mathrm{TranslationTokens}}
\]
（翻訳トークンが 0 の場合は 0 とする）．
...
\begin{itemize}
  ...
  \item 2インデックス（$L_q$ と英語）並列検索を厳密に再現するのではなく，\textbf{同一知識ベースからの検索＋同語除外}によりクロスリンガル条件を構成している．
...
以下は最終版で差し替えるための占位である．

\begin{figure}[H]
  \centering
  \fbox{\parbox{0.85\linewidth}{\centering
  [TODO] EM/F1 の棒グラフ（run\_id\_em\_f1.png を挿入）}}
  \caption{EM/F1 の比較（占位）}
...
\begin{figure}[H]
  \centering
  \fbox{\parbox{0.85\linewidth}{\centering
  [TODO] RLC の棒グラフ（run\_id\_rlc.png を挿入）}}
  \caption{RLC の比較（占位）}
  \label{fig:mkqa_rlc}
\end{figure}
...
\begin{figure}[H]
  \centering
  \fbox{\parbox{0.85\linewidth}{\centering
  [TODO] 翻訳コストと CNBE（run\_id\_cost\_cnbe.png を挿入）}}
  \caption{翻訳コストと CNBE の比較（占位）}
...
\begin{figure}[H]
  \centering
  \fbox{\parbox{0.85\linewidth}{\centering
  [TODO] SAS の棒グラフ（run\_id\_semantic.png を挿入）}}
  \caption{SAS（意味一致スコア）の比較（占位）}
  \label{fig:mkqa_sas}
\end{figure}
...
本研究では，多言語環境における検索拡張生成（RAG）の実運用上の課題──高資源言語への検索バイアス，全文一括翻訳に伴うコスト・情報損失，出力言語の一貫性低下，証拠帰属の不透明さ──に対して，選択的翻訳を核とする LBRAG（Language-Bridged RAG）を提案した．具体的には，（i）ユーザ言語インデックスと英語インデックスへの並列検索とクロスリンガル再ランキングを統合した多言語混合検索，（ii）関連度・翻訳信頼度・トークンコストに基づく効率比による翻訳対象の貪欲選択，（iii）翻訳後の文単位アライメントと数値・日付・固有名詞などのスロット整合性検証，を組み合わせ，原文との可逆的対応を保ったまま，ユーザ言語で一貫した生成を行う枠組みを構成した．

さらに，従来の正確性指標（EM，F1）に加え，出力が目標言語にどれだけ一貫しているかを測る RLC（Response Language Consistency）と，翻訳トークン数に対する性能向上効率を測る CNBE（Cost-Normalized Bridging Efficiency）を導入し，多言語 RAG に固有の「性能–言語一貫性–コスト」の三者トレードオフを定量化する視点を示した．第4章の評価設計に基づく結果（例示値を含む）からは，LBRAG が全文一括翻訳型手法と比べて翻訳トークンを大幅に抑えつつ，正確性と出力言語一貫性のバランスに優れる可能性，および各コンポーネント（再ランキング，選択的翻訳，アライメント／スロット検証）の寄与が確認できる構成であることが示唆された．

以上より，LBRAG は「部分翻訳」と「証拠の可逆的整合性」を中核に，多言語 RAG の実運用に求められるコスト効率・信頼性・追跡可能性を両立させる設計指針を与える．

\section{課題と展望}

今後の課題と発展可能性を以下にまとめる．
\begin{itemize}
  \item \textbf{翻訳信頼度推定の高精度化}：本稿では $\kappa_i$ を長さ保持率・逆翻訳一致・スロット整合などのヒューリスティクスで推定した．機械翻訳の品質推定（QE）モデルや学習済み判別器を取り込み，学習的に最適化する余地がある．
  \item \textbf{翻訳選択の最適化}：効率比に基づく貪欲法は単純で頑健だが，背包問題としての厳密最適化や，$B$（翻訳予算）へ適応的に応答時間・精度をトレードオフするオンライン最適化の検討が望まれる．
  \item \textbf{アライメントの頑健化}：非ラテン文字・混在表記・例外的表記ゆれに対する文対応の誤対合が残る．多言語 NER／正規化器や，構造整合（テーブル／リスト）を明示的に扱う手法の統合が有効と考えられる．
  \item \textbf{言語制御の強化}：RLC を直接最大化する学習（例：言語制約付きデコーディング，ラティス制約，RLHF による罰則付与）を導入し，code-switching の残余をさらに抑制する．
  \item \textbf{領域横断の一般化}：Wikipedia／ニュース以外（法務・医学・金融等）の専門領域で，数字・単位・固有名詞の厳密性がより重要になる．スロット検証の拡張とドメイン適応の評価が必要である．
  \item \textbf{システム最適化とキャッシュ}：再利用可能な翻訳・アライメント結果のキャッシュ，段落単位の差分更新，クエリ分布を踏まえた事前ブリッジのプリフェッチにより，遅延とコストの同時削減を図る．
  \item \textbf{公平性とユーザ研究}：言語資源の少ない言語での性能・説明可能性・可用性を，利用者調査や計量的公平性指標で評価し，実運用時のアクセシビリティ改善につなげる．
  \item \textbf{学習一体化}：検索・再ランキング・翻訳選択・生成を統合的に学習する方策（例：包括的報酬に基づく強化学習，微分可能近似）を検討し，パイプライン間の誤差伝播を低減する．
\end{itemize}

これらの課題に取り組むことで，LBRAG はより広範な多言語・クロスリンガル環境において，低コストかつ信頼できる知識アクセスを提供するプラットフォームへ発展しうる．
