...
\subsection{身近な例：多言語RAGが役立つ場面}
多言語 RAG の意義は，日常の具体的な場面でも直感的に理解できる．例えば次のような状況では，「質問の言語」と「正しい根拠が書かれている言語」が一致しないことが頻繁に起きる．
\begin{itemize}
  \item \textbf{海外旅行・医療/安全情報}：日本語で「この薬は妊娠中に飲める？」と質問しても，信頼できる一次情報が英語の公的機関サイトや英文の添付文書にしかない場合がある．RAG は根拠文を検索して参照し，ハルシネーションを抑えられる．多言語 RAG はさらに，根拠が英語でも日本語で一貫した回答を返すことを目指す．
  \item \textbf{最新技術・製品トラブル}：「特定のエラーコードの意味」や「仕様変更」は英語のフォーラム/リリースノートが最速で，別言語に情報がまだ出回っていないことがある．このとき全文翻訳は高コストだが，重要箇所だけを訳す選択的翻訳が有効になりうる．
  \item \textbf{学内・行政の手続き}：留学生が母語で質問したい一方，規程や手順は日本語で整備されている．多言語 RAG により，根拠を参照しつつ相手の言語で説明できれば，誤解や手戻りを減らせる．
\end{itemize}
本研究は，このような「根拠は多言語に散在し，回答はユーザ言語で一貫させたい」という現実的要求を，翻訳コストを抑えながら満たす枠組みに着目する．
